...
static void  __attribute__ ((noinline)) sha3_chi_masked(uint64_t st[MASKING_SHARES][25],uint64_t bc[MASKING_SHARES][5])
{
    int i, j, k, l;
    int64_t rand_no=MASKING_SHARES*(MASKING_SHARES-1)/2;
...
static void __attribute__ ((noinline)) KeccakF1600_StatePermute_masked(uint64_t st[MASKING_SHARES][25])
{
    // variables
    int r,i;
    uint64_t bc[MASKING_SHARES][5];
...
static void __attribute__ ((noinline)) keccak_absorb_masked(uint64_t s[MASKING_SHARES][25], uint32_t r, size_t mlen, const uint8_t m[MASKING_SHARES][mlen], uint8_t p)
{
    size_t i, l = 0;
...
static void __attribute__ ((noinline)) keccak_squeezeblocks_masked(uint32_t r, size_t nblocks, size_t tlen, uint8_t t[MASKING_SHARES][tlen], uint64_t s[MASKING_SHARES][25])
{
...
void shake128_masked_HO(size_t outlen, uint8_t output[MASKING_SHARES][outlen], size_t inlen, const uint8_t input[MASKING_SHARES][inlen])
{
#ifdef PROFILE_HASHING
    uint64_t time0 = hal_get_time();
#endif
    size_t i, l;
    uint64_t s[MASKING_SHARES][25] = {0};
    unsigned char t[MASKING_SHARES][SHAKE128_RATE];
...
void sha3_512_masked_HO(uint8_t output[MASKING_SHARES][64], size_t inlen, const uint8_t input[MASKING_SHARES][inlen])
{
#ifdef PROFILE_HASHING
    uint64_t time0 = hal_get_time();
#endif
    ...
    size_t i;
    uint64_t s[MASKING_SHARES][25] = {0};
    uint8_t t[MASKING_SHARES][SHA3_512_RATE];
