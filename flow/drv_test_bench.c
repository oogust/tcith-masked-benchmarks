        ...
        6 : {
       10 :     aes256ctr_xof(&iut_drbg, x, xlen);
		  :     return 0;
        8 : }
		  : 
       ...
       20 : {
        2 :     int fail = 0;
		  : 
       19 :     uint8_t seed[48] = { 0 };
        6 :     uint8_t msg[33 * KATNUM] = { 0 };
		  :     size_t  msg_sz= 0, /*pk_sz = 0,*/ sk_sz = 0, sig_sz = 0, sm_sz = 0;
		  : 
        6 :     uint8_t pk[2 * 32] = { 0 };
        6 :     uint8_t sk[4 * 32] = { 0 };
		  ...
		  :     bool    ok;
		  : 
        5 :     rvkat_info(slh_alg_id(iut));
		  : 
		  :     //  initialize kat seed drbg
      152 :     for (int i = 0; i < 48; i++) {
       48 :         seed[i] = i;
		  :     }
       16 :     aes256ctr_xof_init(&kat_drbg, seed);
		  : 
		  :     //pk_sz = slh_pk_sz(iut);
        5 :     sk_sz = slh_sk_sz(iut);
        3 :     sig_sz = slh_sig_sz(iut);
		  : 
        7 :     for (int count = 0; count < katnum; count++) {
		  : 
        5 :         rvkat_dec("[INFO]\tkat test count", count);
		  : 
		  :         //  create seed and the message
        6 :         aes256ctr_xof(&kat_drbg, seed, 48);
		  :         msg_sz = (count + 1) * 33;
       12 :         aes256ctr_xof(&kat_drbg, msg, msg_sz);
		  : 
        5 :         aes256ctr_xof_init(&iut_drbg, seed);
		  : 
		  :         //  KeyGen
        6 :         test_stack_fill(STACK_FILL_SIZE, STACK_FILL_BYTE);
        3 :         cc = get_clk_ticks();
       21 :         slh_keygen(pk, sk, &iut_randombytes, iut);
        2 :         cc = get_clk_ticks() - cc;
        5 :         stk = test_stack_probe(STACK_FILL_SIZE, STACK_FILL_BYTE);
		  :         //  ---
		  : 
        9 :         clk_label(slh_alg_id(iut), "slh_keygen()", cc, stk);
		  : 
        2 :         if (count < 10) {
       13 :             fail += rvkat_chku32(   "sk", cksum[count][0],
		  :                                     rvkat_cksum(sk, sk_sz));
		  :         }
		  : 
		  :         //  Sign
        4 :         test_stack_fill(STACK_FILL_SIZE, STACK_FILL_BYTE);
        1 :         cc = get_clk_ticks();
        9 :         sm_sz = slh_sign(sm, msg, msg_sz, sk, &iut_randombytes, iut);
        2 :         cc = get_clk_ticks() - cc;
        5 :         stk = test_stack_probe(STACK_FILL_SIZE, STACK_FILL_BYTE);
		  :         //  ---
		  : 
        9 :         clk_label(slh_alg_id(iut), "slh_sign()", cc, stk);
        6 :         memcpy(sm + sm_sz, msg, msg_sz);
        1 :         sm_sz += msg_sz;
		  : 
		  :         if (count < 10) {
       13 :             fail += rvkat_chku32(   "sm", cksum[count][1],
        ...
        4 :         test_stack_fill(STACK_FILL_SIZE, STACK_FILL_BYTE);
        1 :         cc = get_clk_ticks();
       18 :         ok = slh_verify(sm + sig_sz, msg_sz, sm, pk, iut);
        3 :         cc = get_clk_ticks() - cc;
        6 :         stk = test_stack_probe(STACK_FILL_SIZE, STACK_FILL_BYTE);
		  :         //  ---
		  : 
        9 :         clk_label(slh_alg_id(iut), "slh_verify()", cc, stk);
        1 :         if (!ok) {
		  :             fail++;
		  ...
		  : 
		  :         //  flip random bit
        8 :         uint32_t xbit = ((uint32_t) seed[4]) +
		  :                         (((uint32_t) seed[5]) <<  8) +
		  :                         (((uint32_t) seed[6]) << 16) +
		  :                         (((uint32_t) seed[7]) << 24);
       11 :         xbit %= (8 * sm_sz);
        9 :         sm[xbit >> 3] ^= 1 << (xbit & 7);
        8 :         if (slh_verify(sm + sig_sz, msg_sz, sm, pk, iut)) {
		  :             fail++;
        1 :             rvkat_dec("[FAIL]\tslh_verify() flip bit", xbit);
		  :         } else {
        4 :             rvkat_dec("[PASS]\tslh_verify() flip bit", xbit);
		  :         }
		  :     }
		  : 
		  :     return fail;
       18 : }
		  ...
		  :     iut_n = 1;
        7 :     fail += chk_test(   test_iut[iut_n],
