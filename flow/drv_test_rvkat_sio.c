      ...
      112 :         sio_putc('A' - 10 + x);
		  ...
		  : 
       18 :     sio_putc(' ');
      108 :     for (i = 28; i >= 0; i -= 4) {
      310 :         rvkat_out_hexdigit((unsigned) (x >> i));
		  :     }
		  : }
		  : 
       ...
       66 : {
       12 :     int fail = (want == have) ? 0 : 1;
		  : 
       36 :     rvkat_failpass(fail);
       18 :     sio_puts(lab);
       24 :     sio_puts(" ( chk=");
      436 :     rvkat_hexu32(have);
        6 :     if (fail) {
		  :         sio_puts(" WANT=");
		  :         rvkat_hexu32(want);
		  :     }
       ...
       60 : }
        ...
        3 : }
		  ...
		  : {
    69401 :     x ^= ((uint32_t) c) << 24;
		  : 
   277632 :     for (int i = 0; i < 8; i++) {
   555294 :         x = (x << 1) ^ ((-(x >> 31)) & 0x04C11DB7);
		  :     }
		  : 
		  :     return x;
		  : }
        ...
        6 :     uint32_t x = 0;
		  : 
    34696 :     for (size_t i = 0; i < data_sz; i++) {
   901958 :         x = crc32byte(x, data[i]);
		  :     }
        7 :     while (data_sz != 0) {
      369 :         x = crc32byte(x, data_sz & 0xFF);
        7 :         data_sz >>= 8;
		  :     }
       12 :     return ~x;
		  : }
