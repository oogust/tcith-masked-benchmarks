...
void sec_mult(uint8_t c[NSHARES], uint8_t a[NSHARES], uint8_t b[NSHARES]){
    uint8_t r[NSHARES*NSHARES];
    memset(r, 0, NSHARES * NSHARES);
    for(int i = 0; i < NSHARES; i++) {
        for(int j = i+1; j < NSHARES; j++) {
            r[i*NSHARES + j] = rand8();
            r[j*NSHARES + i] = add(add(r[i*NSHARES + j], mult(a[i], b[j])), mult(a[j], b[i]));
        }
    }
    for(int i = 0; i < NSHARES; i++) {
        c[i] = mult(a[i], b[i]);
...
void PINI_sec_mult(uint8_t c[NSHARES], uint8_t a[NSHARES], uint8_t b[NSHARES]){
    ...
    uint8_t r_ij[NSHARES * NSHARES];
    uint8_t z_ij[NSHARES * NSHARES];
    uint8_t s, p0, p1;
    memset(r_ij, 0, NSHARES * NSHARES);
    memset(z_ij, 0, NSHARES * NSHARES);
    for(i = 0; i < NSHARES; i++) {
        offset_i = i * NSHARES;
        for(j = i+1; j < NSHARES; j++) {
            offset_j = j * NSHARES;
            r_ij[offset_i + j] = rand8();
    ...
    for(i = 0; i < NSHARES; i++) {
        ...
        for(j = 0; j < NSHARES; j++) {
            offset_j = j * NSHARES;
            if(j != i) {
                s = add(b[j], r_ij[offset_i + j]);
                p0 = mult((a[i] ^ 1), r_ij[offset_i + j]);
                p1 = mult(a[i], s);
                z_ij[offset_i + j] = add(p0, p1);
            }
        }
    }
    for(i = 0; i < NSHARES; i++) {
